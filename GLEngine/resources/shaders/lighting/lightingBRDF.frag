...
void main()
{
    ...
    vec3 envColor = texture(envMap, TexCoords).rgb;//getSphericalCoord(normalize(envMapCoords))).rgb;
    ...
    else
    {
        ...
        if (directionalMode)
        {
            for (int i = 0; i < lightDirectionalCounter; i++)
            {
                ...
                color += (diffuse * kD + specular) * lightColor;
            }
        }
...
//            vec3 ambientIBL = (diffuseIrradiance * kD);
    ...
    // Position buffer
        ...
        colorOutput = vec4(viewPos, alpha);

    // View Normal buffer
    else if (gBufferView == 3)
        colorOutput = vec4(normal, alpha);

    // Color buffer
    else if (gBufferView == 4)
        colorOutput = vec4(albedo, alpha);

    // Roughness buffer
    else if (gBufferView == 5)
        colorOutput = vec4(vec3(roughness), alpha);

    // Metalness buffer
    else if (gBufferView == 6)
        colorOutput = vec4(vec3(metalness), alpha);

    // Depth buffer
    else if (gBufferView == 7)
        colorOutput = vec4(vec3(depth/1000.0f), alpha);

    // SAO buffer
    else if (gBufferView == 8)
        colorOutput = vec4(vec3(sao), alpha);

    // Velocity buffer
    else if (gBufferView == 9)
        colorOutput = vec4(velocity, 0.0f, alpha);
}
